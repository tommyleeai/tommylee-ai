// ============================================================
// artist-magic-modal.js — 藝術家 Magic Modal
...
window.PromptGen.ArtistMagicModal = (function () {
    ...
    function getData() {
        return window.PromptGen.ArtistMagicData || {};
    }

    const RECENT_KEY = 'atmm_recent';
    ...
    function openArtistMagicModal() {
        ...
        const existing = document.getElementById('atmm-modal');
        ...
        if (state.artistAdvanced) {
            const found = items.find(i => i.value === state.artistAdvanced.selectedValue);
            if (found) selectedItem = found;
        }

        const overlay = document.createElement('div');
        overlay.id = 'atmm-modal';
        overlay.className = 'smm-overlay';
        overlay.innerHTML = `
    <div class="smm-flash"></div>
    <div class="smm-magic-circle">
      <svg viewBox="0 0 200 200"><circle cx="100" cy="100" r="90" fill="none" stroke="#ec4899" stroke-width=".5"/>
      <circle cx="100" cy="100" r="70" fill="none" stroke="#db2777" stroke-width=".3"/>
      <circle cx="100" cy="100" r="50" fill="none" stroke="#f472b6" stroke-width=".3"/>
      <polygon points="100,15 178,145 22,145" fill="none" stroke="#ec4899" stroke-width=".4"/>
      <polygon points="100,185 22,55 178,55" fill="none" stroke="#db2777" stroke-width=".4"/>
      <text x="100" y="105" text-anchor="middle" fill="#ec4899" font-size="8" opacity=".4">✦ ARTIST ✦</text></svg>
    </div>
    <div class="smm-particles" id="atmm-particles"></div>
    <div class="smm-container">
      <div class="smm-dice-overlay" id="atmm-dice-overlay">
        <span class="smm-dice-emoji" id="atmm-dice-emoji">🎲</span>
      </div>
      <div class="smm-header">
        <div class="smm-title-row">
          <div class="smm-title">🧑‍🎨 高級魔法・藝術家大全</div>
          <div class="smm-toolbar">
            <button class="smm-tool-btn" id="atmm-dice" title="隨機選取"><span class="smm-tool-icon">🎲</span> 隨機</button>
          </div>
        </div>
        <div class="smm-search-row">
          <div class="smm-search-wrap">
            <span class="smm-search-icon">🔍</span>
            <input type="text" class="smm-search" id="atmm-search" placeholder="搜尋藝術家 Search artist...">
          </div>
        </div>
      </div>
      <div class="smm-tabs" id="atmm-tabs"></div>
      <div class="smm-body">
        <div class="smm-main">
          <div class="smm-grid-wrap" id="atmm-grid-wrap"><div class="smm-grid" id="atmm-grid"></div></div>
        </div>
        <div class="smm-az" id="atmm-az"></div>
      </div>
      <div class="smm-footer">
        <div class="smm-status" id="atmm-status"></div>
        <div class="smm-actions">
          <button class="smm-btn smm-btn-cancel" id="atmm-cancel">❌ 取消</button>
          <button class="smm-btn smm-btn-apply" id="atmm-apply">✨ 套用魔法</button>
        </div>
      </div>
    </div>
  `;
        ...
        const colors = ['#ec4899', '#f472b6', '#db2777', '#f9a8d4', '#a855f7', '#fff', '#e879f9'];
        for (let i = 0; i < 35; i++) {
        ...
        // Tabs
        const tabsEl = document.getElementById('atmm-tabs');
        TABS.forEach(tab => {
            let count;
            ...
            el.addEventListener('click', () => {
                activeTab = tab.id;
                filterLetter = null;
                document.getElementById('atmm-az').querySelectorAll('.smm-az-letter').forEach(l => l.classList.remove('active'));
        ...
        document.getElementById('atmm-dice').addEventListener('click', function () {
            if (diceAnimating) return;
            diceAnimating = true;
            const overlayDice = document.getElementById('atmm-dice-overlay');
            const diceEl = document.getElementById('atmm-dice-emoji');
            const btn = this;
        ...
        function renderGrid() {
            ...
            document.getElementById('atmm-status').innerHTML = `已載入 ${total} 位藝術家 | 顯示 ${shown}/${total}${selText}`;

            grid.innerHTML = '';
            if (!filtered.length) {
                grid.innerHTML = '<div class="smm-empty">🔍 沒有找到符合的藝術家</div>';
                return;
            }
            const recent = getRecentItems();
            ...
            document.getElementById('atmm-grid-wrap').scrollTop = 0;
        }
    }
    ...
    function playOpenSound() {
        try {
            ...
            [493.88, 587.33, 659.25, 783.99, 880, 987.77, 1046.5, 1174.66].forEach((freq, i) => {
