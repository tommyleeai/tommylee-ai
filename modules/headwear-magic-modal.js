// ============================================
// AI Prompt Generator — Headwear Magic Modal
// openHeadwearMagicModal() 函式
// 模組化自 outfit-magic-modal.js 模版
// ============================================
...
window.PromptGen.HeadwearMagicModal = (function () {
    ...
    // === 讀取外部資料 ===
    function getData() {
        return window.PromptGen.HeadwearMagicData || {};
    }

    // === 📋 最近使用（localStorage）===
    const RECENT_KEY = 'hmm_recent_headwear';
    ...
    function openHeadwearMagicModal() {
        ...
        const existing = document.getElementById('headwear-magic-modal');
        ...
        overlay.id = 'headwear-magic-modal';
        overlay.innerHTML = `
    <div class="hmm-flash"></div>
    <div class="hmm-magic-circle">
    ...
    <div class="hmm-container">
      ...
      <div class="hmm-header">
        <div class="hmm-title-row">
          <div class="hmm-title">🔮 高級魔法・頭飾大全</div>
        ...
        <div class="hmm-search-row">
          <div class="hmm-search-wrap">
            ...
            <input type="text" class="hmm-search" id="hmm-search" placeholder="搜尋頭飾 Search headwear...">
          </div>
        </div>
      </div>
      ...
      <div class="hmm-body">
        <div class="hmm-main">
          <div class="hmm-grid-wrap" id="hmm-grid-wrap"><div class="hmm-grid" id="hmm-grid"></div></div>
        </div>
        <div class="hmm-az" id="hmm-az"></div>
      </div>
      <div class="hmm-bonus" id="hmm-bonus">
        <div class="hmm-bonus-title">⭐ 點選增加特徵 — <span id="hmm-bonus-name"></span></div>
        <div class="hmm-bonus-tags" id="hmm-bonus-tags"></div>
      </div>
  ...
  `;
        ...
        document.getElementById('hmm-dice').addEventListener('click', function () {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    const selectedEl = document.querySelector('.hmm-race-chip.selected');
                    if (selectedEl) {
        ...
        document.getElementById('hmm-apply').addEventListener('click', () => {
            if (selectedItem) {
                ...
                if (recentTab) {
                    recentTab.querySelector('.hmm-tab-count').textContent = getRecentItems().length;
                }
                // ★ 整合關鍵：呼叫主頁面的 selectOption 更新頭飾選擇
                selectOption('headwear', selectedItem.value, { label: selectedItem.name, en: selectedItem.en, value: selectedItem.value });
                // 如果有加分特徵，附加到自訂欄位
                const bonusArr = [...selectedBonuses];
                if (bonusArr.length) {
                    state.customInputs['headwear'] = (state.customInputs['headwear'] || '') +
                        (state.customInputs['headwear'] ? ', ' : '') + bonusArr.join(', ');
                    state.customInputVisible['headwear'] = true;
                }
                generatePrompt();
                saveState();
                renderTabContent();
        ...
        function renderGrid() {
            const grid = document.getElementById('hmm-grid');
            const filtered = getFilteredItems();
            const recent = getRecentItems();
            const total = items.length;
            const shown = filtered.length;
            const selText = selectedItem ? ` | 已選：<b>${selectedItem.name}</b>` : '';
            document.getElementById('hmm-status').innerHTML = `已載入 ${total} 頭飾 | 顯示 ${shown}/${total}${selText}`;

            grid.innerHTML = '';
            if (!filtered.length) {
                grid.innerHTML = '<div class="hmm-empty">🔍 沒有找到符合的頭飾</div>';
                return;
            }

            filtered.forEach(item => {
                const chip = document.createElement('div');
                chip.className = 'hmm-race-chip' + (selectedItem && selectedItem.id === item.id ? ' selected' : '');
                chip.innerHTML = `
                    ...
                    ${item.isHot ? '<span class="hmm-hot-badge">🔥</span>' : ''}
                    ${recent.includes(item.id) ? '<span class="hmm-recent-badge">📋</span>' : ''}
                `;
            ...
            document.getElementById('hmm-grid-wrap').scrollTop = 0;
        }

        function renderBonus(item) {
            const panel = document.getElementById('hmm-bonus');
            const tagsEl = document.getElementById('hmm-bonus-tags');
            const nameEl = document.getElementById('hmm-bonus-name');
            ...
            traits.forEach(trait => {
                ...
                tag.className = 'hmm-bonus-tag' + (selectedBonuses.has(trait.en) ? ' active' : '');
                tag.innerHTML = `<span class="hmm-chip-icon">${trait.icon}</span> ${trait.zh}`;
